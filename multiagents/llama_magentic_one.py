...
from multiagents.llama_utils.LlamaMAOrchestrator import LlamaMAOrchestrator
...
class LlamaMagenticOne(MagenticOneGroupChat):
    """
    LlamaMagenticOne is a specialized group chat class that extends MagenticOne with
    ...
    .. warning::
        LlamaMagenticOne includes enhanced safety measures, but users should still:
    ...
    def __init__(
        ...
        BaseGroupChat.__init__(
            self,
            participants=agents,
            group_chat_manager_name="LlamaMAOrchestrator",
            group_chat_manager_class=LlamaMAOrchestrator,
    ...
    def _validate_client_capabilities(self, client: ChatCompletionClient) -> None:
        ...
        if not all(capabilities.get(cap) for cap in required_capabilities):
            warnings.warn(
                "Client capabilities for LlamaMagenticOne must include vision, " 
        ...
        if not isinstance(client, BaseOpenAIChatCompletionClient):
            warnings.warn(
                "LlamaMagenticOne performs best with OpenAI GPT-4o model either " 
    ...
    ) -> Callable[[], LlamaMAOrchestrator]:
        return lambda: LlamaMAOrchestrator(
            name,
            group_topic_type,
            output_topic_type,
